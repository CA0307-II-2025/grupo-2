...
app.layout = html.Div(id = "main", children = [
    ...
    # Portada
    html.Div([
        html.Img(src = 'assets/ucr_logo.png', style = {'height': '80px', 'display': 'block', 'margin': '0 auto'}),
        html.H1("Dashboard Estadístico - Estadistica Actuarial II", style = {'textAlign': 'center'}),
        html.H3("Proyecto", style = {'textAlign': 'center', 'marginTop': '10px'}),
        html.P("Autores: Joseph Romero - Dixon Montero - Andrey Prado", style = {
            'textAlign': 'center',
            'fontSize': '18px',
            'fontStyle': 'italic',
            'marginBottom': '20px'
...
def mostrar_tab(tab):
    if tab == "tab1":
        ...
        html.Ul([
            html.Li("Plotly Technologies Inc. (n.d.). Dash tutorial. https://dash.plotly.com/tutorial"),
            html.Li("Plotly Technologies Inc. (n.d.). Plotly Python library. https://plotly.com/python/")
        ])
    ])
...
def actualizar_graficos(cat, rango):
    ...
    filtro = datos[(datos["cat"] == cat) & (datos["x"].between(rango[0]//10, rango[1]//10))]
