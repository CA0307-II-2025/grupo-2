...
def load_df():
    ...
    for p in CANDIDATES:
        if os.path.exists(p):
            try:
                ...
                df["tipologia"] = df["tipologia"].astype(str).str.strip().str.upper().str.replace(r"\s+", " ", regex=True)
    ...
    return pd.DataFrame(columns=["ano","evento","categoria","decreto","provincia","canton","latitud","longitud","tipologia","total"]), (last_err or f"No encontré 'datos_crudos.xlsx' en {RAW_DATA_DIR}")
...
if not df.empty:
    df_perdidas_anuales = (
        df.groupby("ano")["total"].sum().sort_index().reset_index()
        .rename(columns={"ano":"Año","total":"Pérdidas"})
    )
    df_perdidas_tipologia = (
        df.groupby("tipologia")["total"].sum().sort_values().reset_index()
...
def logo_component():
    candidates = [
        ...
        "ucr_logo.png","LogoUCR.png"
    ]
    for p in candidates:
        if os.path.exists(p):
            # Si está dentro de assets, usar ruta relativa (servida por Dash)
            if "assets" in os.path.normpath(p).split(os.sep):
                return html.Img(src="assets/"+os.path.basename(p), style={"height":"80px","display":"block","margin":"0 auto"})
            with open(p,"rb") as f:
                b64 = base64.b64encode(f.read()).decode("ascii")
            ext = os.path.splitext(p)[1].lstrip(".").lower()
            return html.Img(src=f"data:image/{ext};base64,{b64}", style={"height":"80px","display":"block","margin":"0 auto"})
    return html.Div("Logo UCR no encontrado", style={"textAlign":"center","color":"#888","fontStyle":"italic"})
    ...
    for root in roots:
        ...
        for name in os.listdir(root):
            ...
            if os.path.isfile(path) and ext in IMG_EXTS:
                ...
                if os.path.getsize(path) < 15000 or any(k in lower for k in ["logo","icon","ucr","thumb","placeholder"]):
                    continue
                ...
                try:
                    ...
                    if len(sig.parameters) == 0:
                        ...
                        if hasattr(res, "to_dict") and hasattr(res, "data"):
                            if (prefixes is None) or any(name.lower().startswith(p) for p in prefixes):
...
app.layout = html.Div(
    id="main",
    children=[
        html.Div([html.H1("Análisis de pérdidas por desastres naturales en Costa Rica", className="titulo")], className="encabezado"),
        html.Div([
            logo_component(),
            html.H1("Dashboard Estadístico - Estadística Actuarial II", style={"textAlign":"center"}),
            html.H3("Proyecto", style={"textAlign":"center","marginTop":"10px"}),
            html.P("Autores: Joseph Romero - Dixon Montero - Andrey Prado", style={"textAlign":"center","fontSize":"18px","fontStyle":"italic","marginBottom":"20px"}),
            html.Div("Fuente de datos: "+str(data_info), style={"textAlign":"center","fontSize":"12px","color":"#666","marginBottom":"10px"}),
            html.Hr(),
        ]),
        html.Div([
            html.Div([
                html.H2("Opciones"),
                dcc.Tabs(
                    id="tabs", value="tab_anio",
                    children=[
                        dcc.Tab(label="Pérdidas por año", value="tab_anio"),
                        dcc.Tab(label="Pérdidas por tipología", value="tab_tip"),
                ...
                dcc.Dropdown(
                    id="dropdown-anio",
                    options=([{"label": str(int(x)), "value": int(x)} for x in sorted(df["ano"].dropna().unique())] if not df.empty else []),
                    value=(int(sorted(df["ano"].dropna().unique())[0]) if not df.empty else None),
                    multi=True,
                ),
                html.Label("Filtrar por tipología:"),
                dcc.Dropdown(
                    id="dropdown-tipologia",
                    options=([{"label": x, "value": x} for x in sorted(df["tipologia"].dropna().unique())] if not df.empty else []),
                ...
                dcc.RangeSlider(
                    ...
                    step=1,
                    marks=({int(a):str(int(a)) for a in sorted(df["ano"].dropna().unique())} if not df.empty else {0:"0",10:"10"}),
                    value=([int(df["ano"].min()), int(df["ano"].max())] if not df.empty else [0,10]),
                ),
                html.Br(),
                html.Div(id="alerta-datos", style={"color":"#b00","fontWeight":"600"} if df.empty else {"display":"none"}),
            ], className="Menu"),
...
def render_tab(tab):
        ...
        return html.Div([
            ...
            html.Div([
                ...
                dash_table.DataTable(
                    ...
                    columns=[{"name":"Estadística","id":"Estadística"},{"name":"Valor","id":"Valor"}],
        ...
        return html.Div([
            ...
            html.Div([
                ...
                dash_table.DataTable(
                    ...
                    columns=[{"name":"Tipología","id":"Tipología"},{"name":"Pérdidas totales","id":"Pérdidas totales"},{"name":"Porcentaje","id":"Porcentaje"}],
                    data=[], page_size=10, style_table={"overflowX":"auto"},
                ),
            ]),
        ])
    if tab=="tab_datos":
        return html.Div([
            ...
            dash_table.DataTable(
                ...
                page_size=12, filter_action="native", sort_action="native", export_format="csv",
    ...
    if tab=="tab_explora":
        ...
        blocks = [html.Div([html.H4(name), dcc.Graph(figure=fig)], style={"marginBottom":"18px"}) for name, fig in figs]
        imgs = find_images_by_keywords(["explora","eda","descriptivo","exploratorio","hist","box","violin","kernel"])
        return html.Div([html.H3("Análisis exploratorio"), *blocks, gallery(imgs)], style={"padding":"0.5rem"})
    if tab=="tab_inferencia":
        figs = collect_plotly_figs(prefixes=["infer","anova","ttest","ic","hipo"])
        blocks = [html.Div([html.H4(name), dcc.Graph(figure=fig)], style={"marginBottom":"18px"}) for name, fig in figs]
        imgs = find_images_by_keywords(["infer","anova","pvalor","p-valor","hipotesis","intervalo"])
        return html.Div([html.H3("Análisis inferencial"), *blocks, gallery(imgs)], style={"padding":"0.5rem"})
    if tab=="tab_modelos":
        figs = collect_plotly_figs(prefixes=["model","regre","clasif","arima","sarima","xg","rf","svm"])
        blocks = [html.Div([html.H4(name), dcc.Graph(figure=fig)], style={"marginBottom":"18px"}) for name, fig in figs]
        imgs = find_images_by_keywords(["modelo","model","regre","clasif","arima","sarima","pronost","xg","rf","svm"])
        return html.Div([html.H3("Modelos"), *blocks, gallery(imgs)], style={"padding":"0.5rem"})
    if tab=="tab_copulas":
        imgs = find_images_by_keywords(["copula","cópula","empirico","simulado","u1","u2","scatter_empirico_u","comparacion_empirico_simulado"])
        return html.Div([html.H3("Cópulas"), gallery(imgs)], style={"padding":"0.5rem"})
    if tab=="tab_galeria":
        all_imgs = find_images_by_keywords([])
        return html.Div([html.H3("Galería (todas las imágenes)"), gallery(all_imgs)], style={"padding":"0.5rem"})
...
def warn_data(tab):
    if df.empty:
        return "No se encontraron datos. Verifica 'datos_crudos.xlsx' en: " + RAW_DATA_DIR
    return ""

@callback(Output("grafico-perdidas-anuales","figure"), Input("slider-anio","value"))
def fig_anual(rango):
    filtered = df_perdidas_anuales[(df_perdidas_anuales["Año"]>=rango[0]) & (df_perdidas_anuales["Año"]<=rango[1])] if not df_perdidas_anuales.empty else pd.DataFrame(columns=["Año","Pérdidas"])
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=filtered["Año"], y=filtered["Pérdidas"], mode="lines+markers", line=dict(color="royalblue", width=2), marker=dict(size=8), name="Pérdidas"))
    fig.update_layout(width=1000, height=500, template="simple_white", title="Pérdidas anuales por desastres naturales en Costa Rica", xaxis_title="Año", yaxis_title="Pérdidas totales", hovermode="x unified")
    return fig

@callback(Output("grafico-perdidas-tipologia","figure"), Input("dropdown-tipologia","value"))
def fig_tipologia(tips):
    if df_perdidas_tipologia.empty:
        return go.Figure(layout=dict(template="simple_white", title="Distribución de Pérdidas por Tipología"))
    ...
    fig = px.bar(filtered, x="Pérdidas", y="Tipología", orientation="h", title="Distribución de Pérdidas por Tipología")
    fig.update_layout(template="simple_white", xaxis_title="Pérdida en Colones", yaxis_title="Tipología")
...
@callback(Output("tabla-resumen-tipologia","data"), Input("dropdown-tipologia","value"))
def tabla_tip(tips):
    if df_perdidas_tipologia.empty:
        return []
    if not tips or (isinstance(tips, list) and len(tips)==0):
        ...
        if not isinstance(tips, list):
            tips = [tips]
        filtered = df_perdidas_tipologia[df_perdidas_tipologia["Tipología"].isin(tips)].copy()
    total = filtered["Pérdidas"].sum()
    filtered["Porcentaje"] = 0.0 if total==0 else (filtered["Pérdidas"]/total*100).round(2)
