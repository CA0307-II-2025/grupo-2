...
from copulas.bivariate import GaussianCopula, StudentTCopula, ClaytonCopula, GumbelCopula, FrankCopula
from copulas.multivariate import GaussianMultivariate  # solo si se requiere multivariante
import warnings
...
df_sel['TOTAL POR TIPOLOGÍA'] = pd.to_numeric(df_sel['TOTAL POR TIPOLOGÍA'], errors='coerce')
df_sel = df_sel.dropna(subset=['CATEGORÍA', 'PROVINCIA', 'TOTAL POR TIPOLOGÍA']).reset_index(drop=True)
...
# ---------------------------
df_agg = df_sel.groupby(['CATEGORÍA', 'PROVINCIA'], dropna=False)['TOTAL POR TIPOLOGÍA'] \
               .sum().reset_index().rename(columns={'TOTAL POR TIPOLOGÍA':'total_agr'})
...
df_agg['y_sim'] = df_agg['total_log'] + np.random.normal(scale=df_agg['total_log'].std()*0.1, size=len(df_agg))
...
# ---------------------------
kendall_tau = stats.kendalltau(data_for_copula['total_agr'], data_for_copula['y_sim']).correlation
spearman_rho = stats.spearmanr(data_for_copula['total_agr'], data_for_copula['y_sim']).correlation
...
# ---------------------------
ranking = sorted([(name, v['aic']) for name,v in results.items()], key=lambda x: x[1] if x[1] is not None else np.inf)
