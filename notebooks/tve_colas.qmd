...
for (name in c('provincia', 'canton', 'tipologia')){
  ...
  df[[name]] <- gsub("Ã³", "ó", df[[name]])
  df[[name]] <- gsub("Ã©", "é", df[[name]])
  df[[name]] <- gsub("Ã", "í", df[[name]])
  df[[name]] <- gsub("Ã±", "ñ", df[[name]])
  df[[name]] <- gsub("Ãº", "ú", df[[name]])
  df[[name]] <- gsub("Ã¡", "á", df[[name]])

}

```

Primero se analizaran las tipologías
...
- Infraestructura : Carreteras, Puentes, Alcantarillas, Ferrovías y Aerodromos
- Energía : Sistemas Eléctricos, Infocomunicaciones y Energía
- Hidrico : Sistemas de Agua, Sistemas de Riego, Ríos y Quebradas
- Social : Social, Primer Impacto, Vivienda, Edificios Públicos y Centros Educativos
...
cat("Distribución de Eventos por Categoría")

print(dist_eventos)
```

Cómo la proporción de Energía, Obras y Otros es bastante baja con respecto a las demás se descartarán

```{r}
df <- df %>% filter(!categoria %in% c('OTROS', 'OBRAS', 'ENERGÍA'))
```
...
dist_dano <- df %>%
  ...
  summarise(
    ...
    proporcion_dano_total = total_danos / sum(datos$total, na.rm = TRUE)
  ) %>%
  arrange(desc(total_danos))

cat("Distribución de Eventos por Monto de Daños")
...
g1 <- ggplot(dist_dano, aes(x = reorder(categoria, -total_danos), y = total_danos)) +
  ...
  geom_text(aes(label = format(total_danos, big.mark = ",", scientific = FALSE)),
            vjust = -0.5, size = 3) +
  labs(title = "Daños Económicos totales por categoría",
       x = "Categoría", y = "Daño Total") +
  theme_minimal() +
...
cat("Distribución de Eventos por Provincia")
...
g_provincia <- ggplot(serie_provincia, aes(x = ano, y = total)) +
  ...
  labs(
    title = "Evolución del daño total anual por provincia",
    x = "Año",
    y = "Daño Total"
  ) +
  theme_minimal() +
...
g_categoria <- ggplot(serie_categoria, aes(x = ano, y = total)) +
  ...
  labs(
    title = "Evolución del daño total por tipología",
    x = "Año",
    y = "Daño Total"
  ) +
  theme_minimal() +
...
g_hist <- ggplot(df, aes(x = total)) +
  ...
  scale_x_log10() +
  labs(
    title = "Distribución de los daños",
...
g_hist_cat <- ggplot(df, aes(x = total)) +
  ...
  facet_wrap(~ categoria, scales = "free_y") +
  labs(
    title = "Distribución de daños por tipología",
    x = "Daños (escala logarítmica)",
    y = "Frecuencia"
  ) +
  theme_minimal() +
...
g_hist_prov <- ggplot(df, aes(x = total)) +
  ...
  facet_wrap(~ provincia, scales = "free_y") +
  labs(
    title = "Distribución de daños por provincia",
    x = "Daños (escala logarítmica)",
    y = "Frecuencia"
  ) +
  theme_minimal() +
...
Se tomarán dos combinaciones de distribuciones para cuerpo + cola

- Combinación #1

Para el cuerpo se usará una distribución $Gamma(\alpha, \beta)$ donde $\alpha$ es el parámetro de forma y su significado dice que si $\alpha<1$ entonces la cola inicial es pesada, $\alpha=1$ es una distribución exponencial y $\alpha>1$ dice que la cola es moderada. Luego el $\beta$ dice que si los valores son altos, la distribución es más concentrada.

- Combinación #2

Para el cuerpo se usará
...
encontrar_umbral_optimo <- function(datos, nombre, u_min = 0.7, u_max = 0.99, puntos = 250, min_excesos = 50){
  ...
  grid()
  legend("topright", legend = paste("Umbral óptimo =", round(u_opt,3)),
...
for (name in unique(df$categoria)) {
  ...
  cat("El Umbral Óptimo para", name, "es", umbral_opt, "\n")
  ...
  cat("Cantidad de excesos:", n_excesos, "- Proporción:", round(p_excesos,4), "\n\n")
}
```
...
# Función que ajusta una distribución dada y devuelve tabla resumen
...
# Simulación de datos ejemplo para cada distribución (100 obs)
set.seed(123)
